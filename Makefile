...
# UPDATED: Parameterize BLOCK_SIZE in data generation
generate-data: clean-local-input
	@echo "Generating data with A: $(A_ROWS)x$(A_COLS), B: $(B_ROWS)x$(B_COLS), block=$(BLOCK_SIZE)"
	...
	@for (( i=0; i<$(numBlockRowsA); i++ )); do \
	  for (( k=0; k<$(numBlockColsA); k++ )); do \
	    block_data=""; \
	    for (( rr=1; rr<=$(BLOCK_SIZE); rr++ )); do \
	      row_data=""; \
	      for (( cc=1; cc<=$(BLOCK_SIZE); cc++ )); do \
...
# UPDATED: Use BLOCK_SIZE and CLUSTER_CORE_NODES in AWS EMR command
